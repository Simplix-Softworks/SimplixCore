    ...
    if(commandSender.hasPermission("simplix.install")) {
      if(strings.length == 0) {
        ...
        if(strings[0].equalsIgnoreCase("install")) {
          ...
          try {
            download(new URL(downloadAddress), new File("./plugins/SimplixCore-BungeeCord.jar"));
          ...
          try {
            ...
            commandSender.sendMessage("There was an error while patching your jar file. Please "
